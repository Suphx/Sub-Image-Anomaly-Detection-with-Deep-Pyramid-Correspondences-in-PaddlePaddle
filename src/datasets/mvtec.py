...
# -------------------------------
# Author: SuphxLin
# CreateTime: 2021/03/26 11:38
# FileName: mvtec.py
# Description: 利用paddle.io.Dataset构造MVTecDataset，用于paddle.io.DataLoader
# Question: 使用T.Compose()构造transform似乎有bug
...
class MVTecDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = T.resize(x, self.resize)
        x = T.center_crop(x, self.cropsize)
        x = T.to_tensor(x)
        x = T.normalize(x, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

        if y == 0:
            mask = paddle.zeros([1, self.cropsize, self.cropsize])
        else:
            mask = Image.open(mask)
            mask = T.resize(mask, self.resize)
            mask = T.center_crop(mask, self.cropsize)
